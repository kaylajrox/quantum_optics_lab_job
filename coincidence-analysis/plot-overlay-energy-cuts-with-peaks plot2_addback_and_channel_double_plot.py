...
# === Peaks to EXCLUDE ===
exclude_peak_numbers = ["5", "9"]  # Peaks you don't want to see at all
...
# === Label counts to uniquify legends ===
label_counts = {}
...
for subfolder in sorted(coic_data_dir.iterdir()):
    ...
    for file_path in sorted(subfolder.glob("*.txt")):
        ...
        # === Uniquify label if duplicates exist ===
...
# === Plot Filtered ===
plot_channel(data_store['Filtered']['CH0'], "CH0 Curves (Filtered)")
plot_channel(data_store['Filtered']['CH1'], "CH1 Curves (Filtered)")

# === Plot Unfiltered if toggled ===
if plot_unfiltered:
    plot_channel(data_store['Unfiltered']['CH0'], "CH0 Curves (Unfiltered)")
    plot_channel(data_store['Unfiltered']['CH1'], "CH1 Curves (Unfiltered)")

# === Plot Raw if toggled ===
if plot_raw:
    plot_channel(data_store['Raw']['CH0'], "CH0 Curves (Raw)")
    plot_channel(data_store['Raw']['CH1'], "CH1 Curves (Raw)")
