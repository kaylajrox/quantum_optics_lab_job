...
data_directory = "20250505_coic_correlation_time_vary"
font_size = 24
font_size_legend = 14
...
for peak_dir in peak_dirs:
    ...
    # Extract correlation window (e.g., 50ns) from directory name
    match_window = re.search(r"(\d+)ns_correlation_window", folder_name)
    correlation_window = match_window.group(1) if match_window else ""
    ...
    for file_path in data_files:
        if "AddBack" in file_path.name:
            ...
            file_groups.setdefault(group_key, []).append((file_path, correlation_time, coincidence, state, second_peak, correlation_window))
...
for (channel, structure), files in file_groups.items():
    ...
    for file_path, correlation_time, coincidence, state, second_peak, correlation_window in files:
        ...
        print(f"Second peak: {second_peak}, Peak: {peak_value:.2f} at index {peak_index}, time = {timestamp:.2f}, Total Counts = {total_counts}")
...
unfiltered_rows.sort(key=sort_key)
raw_rows.sort(key=sort_key)

weighted_rows_sorted = filtered_rows + unfiltered_rows + raw_rows

# Write CSV
weighted_csv_path = script_dir / "peak_summary_weighted_means_counts.csv"
with open(weighted_csv_path, 'w', newline='') as wf:
    writer = csv.writer(wf)
    writer.writerow([
        "Timestamp", "File", "Coincidence", "Second Peak", "Correlation Time", "State",
        "Channel", "Structure", "Weighted Mean Index", "Weighted Mean Time", "Total Counts", "Correlation Window (ns)"
    ])
    for row in weighted_rows_sorted:
        writer.writerow([
            ...
            row["total_counts"], row["correlation_window"]
        ])

print(f"\n✅ Grouped by filter state and sorted by second peak number. Saved to: {weighted_csv_path}")
...
# raw_rows = [r for r in weighted_rows if r["state"] == "raw"]
#
# # Sort within each group by second peak number (ascending), then coincidence, then correlation time
# sort_key = lambda row: (
#     int(row["second_peak"]),
#     row["coincidence"],
#     extract_numeric_time(row["correlation_time"])
...
#     writer.writerow([
#         "Timestamp", "File", "Coincidence", "Second Peak", "Correlation Time", "State",
#         "Channel", "Structure", "Weighted Mean Index", "Weighted Mean Time", "Total Counts"
#     ])
#     for row in weighted_rows_sorted:
#         writer.writerow([
#             row["timestamp_now"], row["file"], row["coincidence"], row["second_peak"], row["correlation_time"],
...
# #         ])
# #
# # print(f"\n✅ Grouped by filter state and saved to: {weighted_csv_path}")
# #
# #
# #
# # # import matplotlib
...
# # #         ])
# # #
# # # print(f"\n✅ Sorted and saved weighted means and total counts to: {weighted_csv_path}")
# # #
# # #
# # #
# # # # import matplotlib
