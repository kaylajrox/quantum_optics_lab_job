...
#
#
# def find_and_label_peaks(data_photon_counts, ax, label, crop_off, vertical_lines=vertical_lines):
#     # Apply cropping
#     data_cropped = data_photon_counts[:-crop_off]
#     x = np.arange(len(data_cropped))
#
#     # Find peaks
...
#             try:
#                 if file.startswith("CH0"):
#                     data_photon_counts = np.loadtxt(file_path, delimiter=',')
#                     label = os.path.relpath(file_path, root_dir)
#                     ch0_by_duration[duration_key].append((data_photon_counts, label))
#                 elif file.startswith("CH1"):
#                     data_photon_counts = np.loadtxt(file_path, delimiter=',')
#                     label = os.path.relpath(file_path, root_dir)
#                     ch1_by_duration[duration_key].append((data_photon_counts, label))
#             except Exception as e:
#                 print(f"Error loading {file_path}: {e}")
#
#
...
#         axes = axes.flatten()
#
#         for idx, (data_photon_counts, label) in enumerate(data_list):
#             ax = axes[idx]
#             # Extract voltage-based title from the label
#             title = extract_voltage_and_title(label)
#
#             # Find peaks and label them on the plot
#             distance, peaks, cropped_data = find_and_label_peaks(data_photon_counts, ax, title, crop_off)
#
